...
if st.session_state.get("youtube_creds") is not None:
    ...
    with st.sidebar:
        ...
        if st.session_state.get("auto_reply_total") > 0:
            st.markdown("<div class='sb-section'>", unsafe_allow_html=True)
            st.markdown("<div class='sb-header'>🚀 CRUISING PROGRESS</div>", unsafe_allow_html=True)
            ...
            st.markdown(f"<div style='font-size:13px; font-weight:600; margin-top:8px;'>Processed: {processed}/{total} ({prog_pct}%)</div>", unsafe_allow_html=True)
            st.markdown(f"<div style='font-size:12px; color:#555; margin-bottom:12px;'>✅ Sent: {success} | ⚠️ Errors: {failed}</div>", unsafe_allow_html=True)
            
            if left > 0:
                if not st.session_state.get("auto_reply_paused"):
                    st.markdown('<div class="stop-btn-wrapper">', unsafe_allow_html=True)
                    st.button("🛑 Stop", key="stop_q_btn_sb", on_click=pause_auto_reply, use_container_width=True)
                    st.markdown('</div>', unsafe_allow_html=True)
                else:
                    c3, c4 = st.columns(2)
                    with c3: 
                        st.markdown('<div class="resume-btn-wrapper">', unsafe_allow_html=True)
                        st.button("▶ Resume", key="resume_q_btn_sb", on_click=resume_auto_reply, use_container_width=True)
                        st.markdown('</div>', unsafe_allow_html=True)
                    with c4: 
                        def cancel_queue():
                        ...
                        st.button("❌ Cancel", type="primary", key="cancel_q_btn_sb", on_click=cancel_queue, use_container_width=True)
            else:
                c3, c4 = st.columns([7, 3])
                with c3:
                    st.markdown('<div class="completed-btn-wrapper">', unsafe_allow_html=True)
                    st.button("✅ Completed", disabled=True, key="comp_btn_sb", use_container_width=True)
                    st.markdown('</div>', unsafe_allow_html=True)
                with c4:
                    def clear_completed():
                        st.session_state["auto_reply_queue"] = []
                        ...
                        st.session_state["auto_reply_paused"] = False
                        st.session_state.pop("queue_warning", None)
                    st.button("❌ Close", type="primary", key="close_q_btn_sb", on_click=clear_completed, use_container_width=True)
            st.markdown("</div><div class='sb-divider'></div>", unsafe_allow_html=True)
        ...
        if autopilot_on:
            autopilot_interval = st.number_input("Scan Interval (Minutes)", min_value=1, max_value=1440, value=st.session_state.get("autopilot_interval", 5), key="autopilot_interval_input")
...
Output ONLY the reply text."""
                                            ...
                                            models_to_try = ["gemini-1.5-flash", "gemini-1.5-flash-latest", "gemini-2.5-flash"]
                                            start_idx = models_to_try.index(active_model) if active_model in models_to_try else 0
                                            
                                            response = None
                                            last_error = None
                                            
                                            for model_name in models_to_try[start_idx:]:
                                                try:
                                                    ...
                                                    err_str = str(inner_e)
                                                    last_error = inner_e
                                                    if "404" in err_str or "NOT_FOUND" in err_str or "503" in err_str or "429" in err_str:
...
Output ONLY the reply text."""
                ...
                err_str = str(e)
                st.session_state["ai_errors"][comment_id] = err_str
                st.session_state["auto_reply_paused"] = True
                st.session_state["queue_warning"] = f"🛑 Queue halted: {err_str}"
                st.rerun()
