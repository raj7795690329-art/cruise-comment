        ...
        with st.container():
            ...
            with col_b:
                ...
                if st.button(btn_text, type="primary", disabled=is_replying_btn, use_container_width=True):
                    # Smart API Priority: Checks UI paste first, then local saved, then secrets
                    ...
                    if active_key:
                        ...
                        st.error("API Key missing. Please input your key in Setup.")
...
Output ONLY the reply text."""
                                    ...
                                    st.error("System configuration missing. Provide an API key.")
                        with ca_mood:
        ...
        if is_replying_active and not st.session_state.get("auto_reply_paused"):
            ...
            try:
                # Prioritizes UI pasted key -> saved local key -> Streamlit Secret
                active_key = st.session_state.get("user_gemini_api_key") or saved_keys.get("api_key") or MASTER_API_KEY
                client = genai.Client(api_key=active_key)
                active_context = st.session_state.get("saved_channel_context", "General vlogging") 
                        ...
                        with st.spinner("Connecting to Gemini..."):
                            try:
                                ...
                                <div style="background-color: #FFF0F0; border: 1px solid #FF3B30; color: #D70015; padding: 12px; border-radius: 6px; font-size: 13px; font-weight: 500; margin-bottom: 12px;">
                                    🛑 Connection failed. Ensure your key has Gemini API access.
                                </div>
                                """, unsafe_allow_html=True)
